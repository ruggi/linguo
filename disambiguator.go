...
func NewDisambiguator(disFile string) *Disambiguator {
	...
	if err != nil {
		panic("Error loading file " + disFile)
	}
		...
		case SENTENCE_BIND:
			{
				...
				for i := 2; i < len(items); i++ {
					if disambiguator.binds[key] == nil {
						disambiguator.binds[key] = set.New()
					}
					disambiguator.binds[key].Add(items[i][1:])
				}
			}
		}
	}
...
func (d *Disambiguator) Analyze(ss *list.List) {
	for s := ss.Front(); s != nil; s = s.Next() {
		for w := s.Value.(*Sentence).Front(); w != nil; w = w.Next() {
			...
			for l := lsens.Front(); l != nil; l = l.Next() {
				...
				if prob > 0 {
					if d.wnids[id] != nil {
						println(id, w.Value.(*Word).getForm(), d.wnids[id].lemma, d.wnids[id].gloss, d.wnids[id].domain)
					}
				}
			}
		}
